...
const botTemplates = [
  { icon: Mail, label: 'Email Processor', desc: 'Parse, categorize, and route incoming emails automatically', gradient: 'from-blue-500 to-cyan-400' },
  { icon: FileText, label: 'Invoice Bot', desc: 'Extract invoice data and create entries in your ERP system', gradient: 'from-violet-500 to-purple-400' },
  { icon: Database, label: 'Data Migration', desc: 'Move data between systems with validation and error handling', gradient: 'from-emerald-500 to-teal-400' },
  { icon: Globe, label: 'Web Scraper', desc: 'Collect data from websites on a schedule, export to sheets', gradient: 'from-amber-500 to-orange-400' },
]
    ...
    <>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-12">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease }}
              ...
              <Bot size={16} /> BOT & RPA Engine
            </motion.div>
            <motion.h1 initial={{ opacity: 0, y: 30, filter: 'blur(8px)' }} animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
              transition={{ duration: 0.7, delay: 0.1, ease }}
              className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-white mb-6 leading-tight">
              Automate anything.{' '}
              <span className="bg-clip-text text-transparent" style={{ backgroundImage: 'linear-gradient(135deg, #34d399, #06b6d4, #3b82f6)' }}>
                With bots.
              </span>
            </motion.h1>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900 mb-3">Watch it execute — live</h2>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <div className="p-6 bg-white" style={{ minHeight: 380 }}>
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-emerald-500 to-cyan-400 flex items-center justify-center shadow-lg">
              ...
              {!running && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <span className="text-xs font-bold text-emerald-600">Bot completed in 4.2s — 6/6 steps successful</span>
                </motion.div>
              )}
            </div>
          </motion.div>
      ...
      <section className="py-20" style={{ background: 'linear-gradient(180deg, #f8fafc 0%, #ffffff 100%)' }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900 mb-3">Pre-built bot templates</h2>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {botTemplates.map((t, i) => (
              <motion.div key={t.label} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
                ...
                <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${t.gradient} flex items-center justify-center mb-4 shadow-lg group-hover:scale-110 transition-transform`}>
                  <t.icon size={22} className="text-white" />
                </div>
                <h3 className="text-base font-extrabold text-slate-900 mb-2">{t.label}</h3>
      ...
      <section className="py-20 relative overflow-hidden"
        ...
        <div className="max-w-3xl mx-auto px-4 text-center relative z-10">
          <h2 className="text-3xl sm:text-4xl font-extrabold text-white mb-4">Ready to deploy your first bot?</h2>
          <p className="text-base text-slate-400 mb-8">Automate repetitive tasks and free your team for high-value work</p>
