...
import {
  ...
  Users, FolderOpen, Cpu, Play, Pause,
  ...
  return (
    <section id="showcase" className="homepage-section relative overflow-hidden"
      ...
      <div className="absolute top-0 left-1/3 w-[600px] h-[600px] rounded-full pointer-events-none"
        style={{ background: `${current.color}08`, filter: "blur(100px)", transition: "background 1s" }} />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        {/* ── Main showcase — left menu / right image ── */}
        <motion.div
          ...
          <div className="lg:w-64 flex-shrink-0">
            <div className="flex flex-row lg:flex-col gap-1.5 overflow-x-auto lg:overflow-visible hide-scrollbar rounded-2xl p-2"
              ...
              <button
                ...
                className="flex lg:hidden items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl text-[11px] font-semibold transition-all flex-shrink-0"
                style={{
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      color: isActive ? slide.color : "rgba(255,255,255,0.4)",
                      border: isActive ? `1px solid ${slide.color}30` : "1px solid transparent",
                    }}>
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
                      style={{ background: isActive ? `${slide.color}20` : "rgba(255,255,255,0.05)" }}>
                      <slide.icon size={13} style={{ color: isActive ? slide.color : "rgba(255,255,255,0.4)" }} />
                    </div>
                    {slide.label}
                    {/* Active progress */}
                    {isActive && isPlaying && (
                      <div className="absolute bottom-0.5 left-3 right-3 h-0.5 rounded-full overflow-hidden" style={{ background: `${slide.color}15` }}>
            ...
            <button
              onClick={() => setIsPlaying(!isPlaying)}
              className="hidden lg:flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl text-[11px] font-semibold transition-all w-full mt-2"
              style={{
                background: isPlaying ? "rgba(16,185,129,0.1)" : "rgba(255,255,255,0.05)",
                color: isPlaying ? "#10b981" : "rgba(255,255,255,0.3)",
                border: `1px solid ${isPlaying ? "rgba(16,185,129,0.2)" : "rgba(255,255,255,0.08)"}`,
          ...
          <div className="relative rounded-2xl overflow-hidden flex-1 min-w-0"
            ...
            <div className="h-0.5" style={{ background: "rgba(255,255,255,0.03)" }}>
              <div className="h-full transition-all duration-100 ease-linear"
                style={{ width: `${((active / slides.length) * 100) + (progress / slides.length)}%`, background: current.color }} />
            </div>
          </div>
        </motion.div>
      </div>
